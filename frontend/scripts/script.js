...
const dataset_url =
  "https://datasets-server.huggingface.co/info?dataset=bluuebunny%2Farxiv_abstract_embedding_mxbai_large_v1_milvus_binary";
...
fetch(dataset_url)
  ...
  .then(
    (result) =>
      (myTextArea.placeholder = `Search ${result.dataset_info.default.splits.train.num_examples.toLocaleString()} arXiv papers`),
  );
...
function perform_search(event) {
  ...
  const year_filter_value =
    yearDropdown.options[yearDropdown.selectedIndex].value;
  console.log("year_filter_value");
  console.log(year_filter_value);
  const category_filter_value =
